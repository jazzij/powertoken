...
from common import _get_sqlite_timestamp, to_datetime, DB_PATH
...
def get_users_with_current_activities():
	...
	for activity in activities:
		...
		if (st - timedelta(minutes=15)).time() <= now <= (st + timedelta(minutes=15)).time()
			or (et - timedelta(minutes=15)).time() <= now <= (et + timedelta(minutes=15)).time():
